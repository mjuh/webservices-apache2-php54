...
sh = dash.overrideAttrs (_: rec {
  ...
  rootfs = mkRootfs {
      ...
      src = ./rootfs;
      inherit curl coreutils findutils apacheHttpdmpmITK apacheHttpd mjHttpErrorPages php54 postfix s6 execline;
...
# PHP_INI_SCAN_DIR = ":${rootfs}/etc/phpsec/$SECURITY_LEVEL";
dockerArgHints = {
    init = false;
    read_only = true;
    network = "host";
